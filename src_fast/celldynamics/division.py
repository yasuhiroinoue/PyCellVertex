...
def update_centers(state: GlobalState) -> None:
    ...
    pos = np.array([[vp.loc[0].x, vp.loc[0].y, vp.loc[0].z] for vp in state.p_v], dtype=np.float64)
    if len(pos) == 0:
        return
    ...
    center_contrib = (v_curr + v_next) / 3.0 * cross_z[:, np.newaxis]

    cell_indices = np.repeat(np.arange(num_c, dtype=np.int32), counts)

    area_all = np.zeros(num_c, dtype=np.float64)
    np.add.at(area_all, cell_indices, cross_z)

    center_sum = np.zeros((num_c, 3), dtype=np.float64)
    np.add.at(center_sum[:, 0], cell_indices, center_contrib[:, 0])
    np.add.at(center_sum[:, 1], cell_indices, center_contrib[:, 1])
    np.add.at(center_sum[:, 2], cell_indices, center_contrib[:, 2])

    eps = 1e-12
    zero_area_mask = np.abs(area_all) < eps

    if np.any(zero_area_mask):
        v_sum = np.zeros((num_c, 3), dtype=np.float64)
        np.add.at(v_sum[:, 0], cell_indices, v_curr[:, 0])
        np.add.at(v_sum[:, 1], cell_indices, v_curr[:, 1])
        np.add.at(v_sum[:, 2], cell_indices, v_curr[:, 2])
        safe_counts = np.maximum(counts, 1)
        v_avg = v_sum / safe_counts[:, np.newaxis]

        safe_area = np.where(zero_area_mask, 1.0, area_all)
        center = center_sum / safe_area[:, np.newaxis]
        center[zero_area_mask] = v_avg[zero_area_mask]
    else:
        center = center_sum / area_all[:, np.newaxis]

    for i, cp in enumerate(state.p_c):
        if counts[i] > 0:
            cp.center.x = float(center[i, 0])
            cp.center.y = float(center[i, 1])
            cp.center.z = float(center[i, 2])
    ...
    for cidx, cp in enumerate(state.p_c):
        ...
        while len(tmp_li) < len(cp.li):
            ...
            if not found:
                ...
                logging.error(
                    f"Topology breakdown in sort_counter_clockwise for cell {cidx} at vertex {cur}. Cell edges: {cp.li}, sorted so far: {tmp_li}"
                )
                break
        area = 0.0
        if len(tmp_vi) >= 3:
...
def cell_division(state: GlobalState, cellula_idx: int, axis: Vec3) -> None:
    ...
    l1n = Line(
        (v1_idx, epoint[1]),
        [x for x in p0.ci if x != cellula_idx] + [c1_idx],
        p0.K1_LENGTH,
        p0.K1_PCP_LENGTH,
        p0.K2_LENGTH,
        p0.LENGTH_EQ,
    ...
    l2n = Line(
        (v2_idx, epoint[2]),
        [x for x in p1.ci if x != cellula_idx] + [c1_idx],
        p1.K1_LENGTH,
        p1.K1_PCP_LENGTH,
        p1.K2_LENGTH,
        p1.LENGTH_EQ,
